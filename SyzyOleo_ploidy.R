...
counts2 <- read_dart_counts_csv_faster('SyzyOleo/dart_raw/Report_DSyz22-7444_SNPcount_3.csv', # import readcount data 
                                       minAlleleCount=1, 
                                       minGenotypeCount=0)
...
a_function <- function(dms){
  ...
  for(i in 1:length(species)){
    dmsx <- remove.by.list(dms, m2[(m2$sp %in% paste(species[i])),] %>%.$sample) %>% remove.by.maf(., 0.03)
    test2 <- count_subsetter(dmsx, counts2, 0.05)
